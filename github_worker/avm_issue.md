...
你正在处理一个 **Azure Verified Module (AVM)** 仓库的 GitHub Issue。
...
如果你确实需要原始的 `name` / `desc` / `firstLine`（例如要扫描描述里的额外链接或截图），调用 gateway 注册的 `trello_card_get` 工具：
...
例如：`https://github.com/Azure/terraform-azurerm-avm-res-app-containerapp/issues/118`
- **仓库名**：`Azure/terraform-azurerm-avm-res-app-containerapp`
- **Issue 号**：`118`
...
| **AVM 模块规范** | Azure Verified Modules 的官方规范和指南 | `azure.github.io/Azure-Verified-Modules/...` |
...
### 2.5 AzAPI-模拟-AzureRM 模式模块的参考文档（强制）

部分 AVM 模块使用 `azapi` provider 作为底层实现，但变量的命名、结构和语义刻意模拟 `azurerm` resource schema（即 AzAPI-模拟-AzureRM 模式）。这种模式下的模块有特殊的实现规范和约束。

**当 Agent 识别出当前仓库的模块属于 AzAPI-模拟-AzureRM 模式时**（判定方法：核心资源使用 `azapi_resource`，但变量按 `azurerm` 风格设计），**必须**在进行任何代码修改、方案设计或修复验证之前，先阅读以下参考文档：

**参考文档**：[AVM Replicator Executor Guide](https://raw.githubusercontent.com/lonegunmanb/avm-replicator-2-azapi/refs/heads/main/replicator/executor.md)

该文档记录了用 AzAPI 模拟 AzureRM 行为时的关键实现规范，Agent 需要特别注意以下要点：

1. **精确复制原则**：必须精确复制 AzureRM Provider 的所有行为（验证、默认值、ForceNew、类型转换等），不允许"更安全"或"更简单"的替代方案。
2. **AzAPI Provider 2.0+**：使用原生 Terraform 对象传递 `body`，不需要 `jsonencode()`/`jsondecode()`。
3. **验证规则复制**：AzureRM Provider 内置的验证在 AzAPI 中不存在，必须在 `variables.tf` 的 `validation` 块中显式复制所有验证规则（值约束、跨字段约束、自定义逻辑）。
4. **默认值复制**：AzureRM Provider 的 `Default` 值必须在变量定义中显式设置。
5. **ForceNew 处理**：通过 `replace_triggers_external_values` + `data "azapi_resource"` 复制 AzureRM 的 ForceNew 行为。
6. **敏感字段处理**：敏感/WriteOnly 字段必须放在 `sensitive_body` 中，使用独立的 ephemeral 变量和 version 变量。
7. **body 结构**：只有 `type`、`name`、`location`、`parent_id`、`identity`、`tags` 等特定字段放在 `azapi_resource` 根级，其他所有字段（包括 `zones`、`sku` 等根级 API 字段）都放在 `body` 内。
8. **跨变量验证**：Terraform 1.9+ 支持 `variable` validation 块引用其他变量，跨变量验证应在 `variables.tf` 中实现。

Agent 在分析问题、设计方案或实施修复时，应参照该文档中的模式和约束，确保所有改动与模块现有代码风格和架构完全一致。

> **适用范围**：本规则适用于所有 Issue 类型（Bug、功能请求、问题/反馈等），只要目标模块被识别为 AzAPI-模拟-AzureRM 模式即触发。
...
| 类别 | 定义 | 典型信号 | 参见
|---|---|---|---|
| Bug | 预期行为未被满足且可以复现。 | "不工作"、回归、错误输出 | {{avm_issue_bug.md}} |
| 功能请求 | 请求新功能或增强现有功能。 | "请支持"、"添加选项" | {{avm_issue_feature_request.md}} |
| 破坏性变更 / 行为变更 | 请求的变更可能影响现有用户或显著改变行为。 | 需要迁移、兼容性风险 | |
| 文档 | 文档/示例/README 有误、缺失或不清晰。 | "文档不清楚"、示例不匹配 | |
| 问题 / 反馈 | 使用疑问、设计澄清或一般性反馈，没有直接的缺陷证据。 | "我应该怎么"、"这是预期行为吗" | {{avm_issue_question.md}} |
| 安全漏洞 | 安全漏洞、敏感信息泄露或策略风险行为。 | 密钥泄露、权限提升、安全问题 | {{avm_issue_security.md}} |
